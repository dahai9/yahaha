...
if __name__ == '__main__':
    with open('./data.yaml','r',encoding='utf-8') as fp:
        infos = yaml.load(fp.read(),yaml.Loader)
    str = ''
    for info in infos:
        str += info['phone']+'#'+info['appid']+'#'+info['token_online']+'&'
        cug = CUG(info['phone'],info['appid'],info['token_online'])
        cug.main()
    print(str)
